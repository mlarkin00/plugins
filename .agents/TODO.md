...
- [ ] **[P2]** Decide how much staleness the `active-skills` sync may carry when the `repository_dispatch` does not arrive. On 2026-08-06 six pushes to `mlarkin00/active-skills` between 18:35 and 19:44 UTC produced **zero** `Notify marketplace` runs (last run #52, 18:08Z, head `ef7475d`; the workflow is `state: active` and the repo is public and not disabled), so the marketplace never learned of them — including `d881a18`, which removed six skills. GitHub-side, so not fixable here; the only fallback is the 06:17 UTC daily poll, which bounds staleness at ~24h. Options: raise the poll frequency, or have the poll compare the mirrored skill set against the source tree and fail loudly on drift. Related evidence: `.agents/wiki/testing/rsync-protects-excluded.md`.
