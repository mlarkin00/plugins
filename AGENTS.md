...
* [claude-code](.agents/wiki/claude-code/index.md) - Contains 3 entries: An agent whose frontmatter is not valid YAML is skipped silently, Updating an installed Claude Code plugin, Injected context is stored as typed records, not as rendered system-reminder text.
